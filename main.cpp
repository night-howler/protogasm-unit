...
#define BRIGHTNESS 20 //Subject to change, limits current that the LEDs draw
...
#define LONG_PRESS_MS 400 //ms requirements for a long press, to move to option menus
...
void run_auto() {
  ...
  else if ((pressure - avgPressure) * 1.5 > pLimit && motSpeed > 0) {
    motSpeed-=(((pressure-avgPressure)*1.5)-pLimit)*50/avgPressure;
  }
  else if (motSpeed < (float)maxSpeed) {
    motSpeed += motIncrement;
  }
  else if (motSpeed > (float)maxSpeed) {
    if (motSpeed - maxSpeed < 1) {
      motSpeed = maxSpeed;
    } else {
      motSpeed--;
    }
...
void run_opt_limit() {
  sensitivity = encLimitRead(0, (3 * NUM_LEDS) - 1);
  //Reverse "Knob" to map it onto a pressure limit, so that it effectively adjusts sensitivity
  pLimit = map(sensitivity, 0, 3 * (NUM_LEDS - 1), 600, 1); //set the limit of delta pressure before the vibrator turns off
}

//Setting menu for adjusting the maximum vibrator speed automatic mode will ramp up to
void run_opt_speed() {
  int knob = encLimitRead(0, NUM_LEDS - 1);
  maxSpeed = map(knob, 0, NUM_LEDS - 1, 0., (float)MOT_MAX);
}
...
uint8_t check_button() {
  ...
  static bool ignoreBtnUntilReleased = false;
  ...
  if (ignoreBtnUntilReleased == false ) {
    if (thisBtn == ENC_SW_DOWN && (millis() - keyDownTime) > LONG_PRESS_MS) {
      btnState = BTN_LONG;
      ignoreBtnUntilReleased = true;
    } else if (thisBtn == ENC_SW_UP && lastBtn == ENC_SW_DOWN) { //there was a keyup
      btnState = BTN_SHORT;
    }
  }

  if (thisBtn == ENC_SW_UP) {
    ignoreBtnUntilReleased = false;
  }
  ...
  if (millis() % period == 0) {
    ...
    for (uint8_t i = OVERSAMPLE; i; --i) {
      ...
      if (i) {     // Don't delay after the last sample
        delayMicroseconds(100);  // Spread samples out a little
      }
    }
